...
from finetune_models import BertForMultilabelSequenceClassification, BertForSNPClassification, SubBertForSequenceClassification, SubBertForSNPClassification
...
MODEL_CLASSES = {
    ...
    "dnamlc": (BertConfig, BertForMultilabelSequenceClassification, DNATokenizer), 
    "dnasnp": (BertConfig, BertForSNPClassification, DNATokenizer),
    "dnasub": (BertConfig, SubBertForSequenceClassification, DNATokenizer) ,
    "dnasubsnp": (BertConfig, SubBertForSNPClassification, DNATokenizer) 
}
...
def predict(args, model, tokenizer, prefix=""):
    ...
    for pred_task, pred_output_dir in zip(pred_task_names, pred_outputs_dirs):
        pred_dataset = load_and_cache_examples(args, pred_task, tokenizer, evaluate=False, test=True)
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, test=False):
    ...
    cached_features_file = os.path.join(
        ...
        "cached_{}_{}_{}_{}".format(
            ...
            "test" if test else "dev" if evaluate else "train", ### changed by TXL
    ...
    elif output_mode == "multilabelclassification":
        all_labels = torch.tensor([f.label for f in features], dtype=torch.float)
...
def main():
    ...
    parser.add_argument("--pred_layer", default=11, type=int, help="The layer used for prediction (defalut: 11).")
    parser.add_argument(
        "--predict_scan_size",
        type=int,
        default=1,
    ...
    if not args.do_visualize and not args.do_ensemble_pred:
        ...
        ### Added by TXL: only when using lower layers
        if args.pred_layer != 11:
            config.pred_layer = args.pred_layer
        ...
        ### Added by TXL: used for the SNPClassification model, which needs to consider the k-mer size.
        config.k = tokenizer.kmer
